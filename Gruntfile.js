...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'foxthefox';
    var email     = grunt.option('email')  || 'foxthefox@wysiwis.net';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new musiccast name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/musiccast.png')) {
            fs.renameSync(__dirname + '/admin/musiccast.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/musiccast.html')) {
            fs.renameSync(__dirname + '/widgets/musiccast.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/musiccast/js/musiccast.js')) {
            fs.renameSync(__dirname + '/widgets/musiccast/js/musiccast.js', __dirname + '/widgets/musiccast/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/musiccast')) {
            fs.renameSync(__dirname + '/widgets/musiccast',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Musiccast/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Musiccast'
                        },
                        {
                            match: /foxthefox/g,
                            replacement: author
