    ...
    async onStateChange(id, state) {
        if (state) {
            ...
            if (state && !state.ack) {
                ...
                if (IP.length === 0 || IP.length !== 1) {
                    this.log.debug('NO device found:');
                    this.log.debug(`config items : ${JSON.stringify(this.config.devices)}`);
                    this.log.debug(`returned items : ${JSON.stringify(IP)}`);
        ...
        if (typeof obj === 'object' && obj.message) {
            ...
            switch (obj.command) {
                ...
                case 'jsonreq':
                    ...
                    if (obj.callback) {
                        this.sendTo(obj.from, obj.command, responses, obj.callback);
                    } //responses wird sukzessive mit den get-Aufrufen befüllt
                    wait = true;
                    break;
                default:
	...
	*/
    ...
    async defineMusicSystemInputs(type, uid, sysinputs) {
        ...
        for (let i = 0; i < sysinputs.length; i++) {
            ...
            await this.setObjectNotExistsAsync(`${type}_${uid}.system.inputs.${sysinputs[i].id}.distribution_enable`, {
                type: 'state',
                common: {
            ...
            await this.setObjectNotExistsAsync(`${type}_${uid}.system.inputs.${sysinputs[i].id}.account_enable`, {
                type: 'state',
                common: {
            ...
            await this.setObjectNotExistsAsync(`${type}_${uid}.system.inputs.${sysinputs[i].id}.play_info_type`, {
                type: 'state',
                common: {
                    name: 'play info type',
                    type: 'string',
    ...
    async getMusicZoneInfo(ip, type, uid, zone) {
        ...
        try {
            ...
            if (att.response_code === 0) {
                ...
                if (
                    !responses.find(o => o.device === `${devtype}_${devuid}` && o.request === `/${zone_name}/getStatus`)
                ) {
                    responses.push(resp);
                }
            ...
            if (att.response_code === 0) {
                for (let i = 0; i < att.system.zone_num; i++) {
                    ...
                    if (att.zone[i].func_list.indexOf('link_audio_quality') !== -1) {
                        await this.setStateAsync(`${devtype}_${devuid}.${zone_name}.link_audio_quality_list`, {
                            val: JSON.stringify(att.zone[i].link_audio_quality_list),
                            ack: true,
                        });
                    }
                    ...
                    if (att.zone[i].func_list.indexOf('actual_volume') !== -1) {
                        await this.setStateAsync(`${devtype}_${devuid}.${zone_name}.actual_volume_mode_list`, {
                            val: JSON.stringify(att.zone[i].actual_volume_mode_list),
                            ack: true,
                        });
                    }
                }
                ...
                for (let i = 0; i < att.system.input_list.length; i++) {
                    ...
                    await this.setStateAsync(
                        `${devtype}_${devuid}.system.inputs.${att.system.input_list[i].id}.distribution_enable`,
    ...
    async getMusicNetusbRecent(ip, type, uid) {
        ...
        try {
            ...
            if (att.response_code === 0) {
                ...
                this.log.debug(`failure getting Netusb recent info from  ${devip} : ${this.responseFailLog(result)}`);
            }
        } catch (err) {
            if (err.message.includes('connect EHOSTUNREACH')) {
                this.log.debug(
    ...
    async getMusicNetusbPreset(ip, type, uid) {
        ...
        try {
            ...
            if (att.response_code === 0) {
                ...
                this.log.debug(`failure getting Netusb preset info from  ${devip} : ${this.responseFailLog(result)}`);
            }
        } catch (err) {
            if (err.message.includes('connect EHOSTUNREACH')) {
                this.log.debug(
    ...
    async getMusicTunerPreset(ip, type, uid) {
        ...
        try {
            ...
            const att = result;
            if (att.response_code === 0) {
                this.log.debug(`got Common Tuner preset info succesfully from ${devip}with  ${JSON.stringify(result)}`);
        ...
        try {
            ...
            const att = result1;
            if (att.response_code === 0) {
                this.log.debug(`got FM Tuner preset info succesfully from ${devip}with  ${JSON.stringify(result1)}`);
                const resp = {
                    device: `${devtype}_${devuid}`,
        ...
        try {
            ...
            const att = result2;
            if (att.response_code === 0) {
                this.log.debug(`got AM Tuner preset info succesfully from ${devip}with  ${JSON.stringify(result2)}`);
                const resp = {
                    device: `${devtype}_${devuid}`,
        ...
        try {
            ...
            const att = result3;
            if (att.response_code === 0) {
                this.log.debug(`got DAB Tuner preset info succesfully from ${devip}with  ${JSON.stringify(result3)}`);
                const resp = {
                    device: `${devtype}_${devuid}`,
        ...
        try {
            ...
            if (att.response_code === 0) {
				...
				*/
                ...
                this.log.debug(`failure getting Distibution info from  ${devip} : ${this.responseFailLog(result)}`);
            }
        } catch (err) {
            if (err.message.includes('connect EHOSTUNREACH')) {
